...
export default function EditForm() {
    const {updateExpense, editingExpense, startEditing} = useExpenseContext();

    const formik = useFormik({
        initialValues: {
            expenseName: editingExpense.expenseName,
            cost: editingExpense.cost,
            addedOn: editingExpense.addedOn
        },
        ...
        <form onSubmit={formik.handleSubmit} className="bg-gray-100/30 fixed top-0 right-0 left-0 z-50 w-full h-full">
            <div className="font-sans max-w-xs mx-auto bg-white my-18 border border-gray-100 rounded-lg pb-8 px-5 py-3 shadow-lg sm:max-w-sm">
                <div className="flex justify-between items-center mb-5">
                    <span className="text-xl font-semibold text-gray-800">Edit an expense</span>
